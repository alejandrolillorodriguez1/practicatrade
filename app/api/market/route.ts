...
function searchScore(quote:any,query:string,index:number){
 ...
 return (symbol===wanted?500:0)+(name===wanted?450:0)+(name.startsWith(wanted)?250:0)+(symbol.startsWith(wanted)?180:0)+(quote.quoteType==='EQUITY'?80:quote.quoteType==='ETF'?30:50)+(preferredExchanges.includes(quote.exchange)?45:0)+(!String(quote.symbol).includes('.')?20:0)-index;
}
